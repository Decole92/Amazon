...
{items?.length === 0 ? 'Your Shopping Cart is empty.' : 'Shopping Cart' }
     ...
     {items?.map((product, index) => (
          ...
          <CheckoutProducts key={index} id={product.id} quantity={product.quantity} />
    ...
    <div className={`flex flex-col p-10 ${items?.length > 0 && 'shadow-md'}`}>
        {items?.length > 0 && (
            <div className='flex flex-col'>

                <h2 className='whitespace-nowrap'>Subtotal ({items.length} items): {" "}
                
                <span className='font-bold'> {getTotalCost().toFixed(2) * 25} Kč</span>
